...
function Deployer() {
  ...
  // This effect runs to update the balance and status when the connection changes
  useEffect(() => {
    // We only fetch the balance if we are connected and have a provider
    if (isConnected && walletProvider) {
      const fetchBalance = async () => {
        try {
          const provider = new ethers.providers.Web3Provider(walletProvider);
          const balanceWei = await provider.getBalance(address);
          // Format the balance to 4 decimal places for display
  ...
  }, [isConnected, walletProvider, address]); // This effect depends on these values
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      if (!walletProvider) {
        setStatus('Error: Wallet provider not ready. Please try again.');
      ...
      console.error(error);
      // This new, more robust check handles the rejection error correctly
  ...
  // This new function handles disconnection gracefully
  const handleDisconnect = async () => {
    await disconnect();
    // Forcing a page reload is the simplest way to ensure a clean state
