...
function Deployer() {
  ...
  }, [isConnected, walletProvider, address, chainId]); // Added chainId to dependencies
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      if (!walletProvider) {
        setStatus('Error: Wallet provider not ready. Please click again.');
      ...
      console.error(error);
      // --- CHANGE 1: Improved rejection message ---
      if (error.code === 'ACTION_REJECTED' || error.code === 4001) {
        setStatus('❌ Transaction Rejected by User');
      } else {
        setStatus(`An unexpected error occurred. See console for details.`);
      }
    } finally {
      setIsDeploying(false);
    }
  ...
  // Disconnect and reload the page for a clean state
  ...
  return (
    <div className="container">
      ...
      <footer className="footer">
        ...
        {/* --- CHANGE 2: Added disclaimer --- */}
        <p className="disclaimer">
          This tool is an independent project and is not affiliated with the official Zama team.
        </p>
      </footer>
    </div>
  );
